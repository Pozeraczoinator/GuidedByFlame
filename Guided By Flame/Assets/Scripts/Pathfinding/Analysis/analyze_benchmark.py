    ...
    for ax, scenario in zip(axes, DYNAMIC_SCENARIOS):
        for algorithm in ALGORITHMS:
            ...
            ax.scatter(
                ...
                mean(r.path_ratio for r in subset if r.path_ratio is not None),
        ...
        ax.set_ylabel("Długość ścieżki / długość referencyjna")
    show_y_axis_on_all(axes)
...
def default_project_csv() -> Path:
    ...
    return project_root / "benchmark_results_official.csv"
